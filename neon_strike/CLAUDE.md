...
- `controllers/main.py` — `NeonStrikeController` (`http.Controller`). `GET /neon` (`auth="public"`, `website=True`, `sitemap=False`) renders `neon_strike.page` and makes sure the session token exists. JSON routes `auth="public"` (`/neon/create|join|start|input|state|score|solo_score|leave|scores`) operating with `sudo()` and resolving the player from `request.session["neon_token"]`.
