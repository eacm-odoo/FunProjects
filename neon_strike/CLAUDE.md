...
- `static/src/js/colossi.js` — The **5 colossal bosses** as data (`COLOSSI`, `colossusForWave`): sprite, tint, logical width (all wider than the 680 arena), camera `zoom`, hit points, score value and the blurb reused by the glossary. Their AI lives in `_updateColossus` keyed by index, same order. **AEGIS-01 no longer uses `speed`**: it flies `aegis_motion.js`, and `descend.restY` there mirrors its `y`. **HYDRA-07's two side heads are separate targets** with hull points of their own (`HYDRA_HEAD` in the engine), so its entry also carries `heads`, `parts` and the director's clock.
