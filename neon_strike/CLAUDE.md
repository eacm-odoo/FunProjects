...
- **Camera**: `this.zoom` is a **render-only** concern (plus `_ptr`, which must invert the same `scale * zoom`). `_updateCamera` reads the zoom off the live colossus, so the guest reaches the same value from the snapshot without sending a camera. The HUD is drawn in its own unzoomed transform (`hudOx/hudOy`) so it never shrinks.
- **The playable field is dynamic**: `this.field` (also read off the colossus, `COLOSSI[k].field`, 1.30-1.45) widens the arena while one is alive, and `fx0/fx1/fy0/fy1` are the live bounds — **they can be negative**. Every "did it leave the field" check (ship clamp, bullets, enemy bullets, rocks bouncing, capsules, enemies falling out) and the AEGIS curtain / VULCAN rock barrage use those, never `0..W/H`. Both `zoom` and `field` snap to their target once within 0.002, otherwise the exponential easing leaves the walls fractionally off forever.
- **Supply drops**: a boss fight kills the capsule flow (no small fry, no drops). `_updateSupply` drops one every ~7 s while `_bossPresent()`, and every boss sheds one per 25% of health lost (`e.dropAt`). Both use `dropPup(x, y, true)`, which is weighted towards triple shot and shield.
