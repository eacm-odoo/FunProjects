...
- `static/src/js/sprites.js` — Banco de sprites pixel art. Cada sprite es una rejilla de caracteres (1 char = 1 píxel lógico); los simétricos se escriben a media anchura y se espejan (`mir: true`). Los índices `4/5/6` son **tinte**: se recolorean al dibujar con el color de la nave/enemigo, así un mismo chasis sirve para los 4 slots y para variantes de enemigo. API: `sprite(name, tint, px, flash)` (canvas rasterizado, cacheado por nombre+color+escala+flash), `drawSprite(g, name, x, y, {tint, px, flash, rot, alpha})`, `spriteSize(name)`, `pxFor(name, target)`. No importa nada: se puede usar fuera de Odoo. Sprites: `ship0..3`, `drone0/1`, `speedy0/1`, `tank0/1`, `sniper0`, `kami0`, `boss0`, `pupT/pupS/pupB/pupL`, `rock0/1`. Los power-ups llevan el glifo (T/S/bomba/+) dibujado en la propia rejilla, así que el nombre del sprite es `"pup" + p.t` y el tinte sale de `PUP_COLORS`: si añades un tipo de power-up hay que tocar los dos sitios.
...
- **Sprites y multijugador**: la variante de chasis (`e.v` en enemigos, `rk.v` en asteroides) se sortea al crear la entidad y **viaja en el snapshot**; si no, host y guest verían chasis distintos. Igual con `rt` (rotación del kamikaze) y `am` (carga del francotirador), que el guest no puede recalcular porque no simula. Los asteroides ya no llevan el polígono `spin` en el snapshot: se dibujan con `rock0/rock1`.
- **Orientación de los sprites**: las naves de jugador miran arriba y los enemigos abajo, tal cual está la rejilla. Al rotar el kamikaze se resta `Math.PI/2` porque su chasis apunta a +Y, no a +X.
