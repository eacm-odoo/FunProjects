...
class BattleshipController(http.Controller):
    ...
    def _token(self):
        """Stable per-browser token of an anonymous player (created on first use)."""
    ...
    def _game(self, game_id):
        ...
        one's board.
        """
        domain = [("id", "=", self._as_int(game_id, 0, 2 ** 31))]
        if self._is_public():
    ...
    def state(self, game_id=None, **kwargs):
        return self._game(game_id).read_state()
    ...
    def place_ship(self, game_id=None, side=None, index=None, cell=None, direction=None, **kwargs):
        return self._game(game_id).action_place_ship(
            self._as_choice(side, SIDES),
